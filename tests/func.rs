...
fn functional_tests() {
    ...
    let mut cpu: Cpu6502<NMOS6502, FastMapBus> = Cpu6502::new();

    let mut f = File::open("tests/func_tests/6502_functional_test.bin").unwrap();
    let mut buf = Vec::new();
    f.read_to_end(&mut buf).unwrap();
    let mut page = 0;
    for mut page_data in buf.as_chunks_mut::<0x1000>().0 {
        bus.map_ram_page_from_slice(0, page, &mut page_data);
        page += 1;
    }
...
fn extended_opcode_tests() {
    let mut bus = FastMapBus::new();
    let mut cpu: Cpu6502<CMOS65C02, FastMapBus> = Cpu6502::new();
    ...
    while cpu.pc != SUCCESS_ADDR {
        ...
        cycles += 1;
        if cycles % 1000000 == 0 {
            println!("curr pc = {pc:#06X} x = {}", cpu.x);
        }
        if pc == cpu.pc {
            same_pc_counter += 1;
            if same_pc_counter > 10 {
                assert!(false, "HIT A TRAP AT {pc:#06X}")
            }
        } else {
            same_pc_counter = 0;
        }
